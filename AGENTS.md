...
| `Detection/DoorMonitorAnalyzer.cs` | 门状态监测（关门基准比对+亮度归一化抗光照+人遮挡排除+防抖状态机）；仅检测线程串行调用，SetBaseline 由 UI 线程调（引用原子替换） |
...
- **`CHANGELOG.md`**：功能/修复完成后在顶部新增或更新版本小节，写明"改动范围、为什么这么改、优化点"三部分。改动再小也要记，防止现场追溯不到。**版本号约定（v2.9.1 起生效）**：同一天的迭代性修改用修订号（v2.9.1、v2.9.2…追加到当天最新小节或新开修订小节），只有功能级/破坏性变更才递增次版本号——避免版本号一天暴涨失去追溯意义。
- **`README.md`**：每次任务完成后**自行判断**是否需要更新，不要等用户提醒。README 是 GitHub 门面，面向使用者和接入方——凡是**使用者/接入方能感知的信息**变了就要同步：功能特性、构建方式、依赖清单、配置项、目录结构、使用步骤、已知限制。判断方法：改动涉及**能力边界**（平台支持/依赖增减/体积路径/配置结构）时，全局 `rg` 扫 README 里的旧数字、旧平台名、旧路径关键词，不要凭记忆列清单（凭清单会漏掉措辞不同的引用处）。
