...
- SDK 风格 csproj：`.cs` 自动包含，新增源码文件无需登记；配置/模型的复制规则（`Content Include`）集中在 csproj 内维护
- 关键库：
  - OpenCvSharp4 4.10（RTSP 视频捕获、Mat 图像处理、绘制）
...
├── Detection/              检测域（零外部业务依赖）：接口/检测器/管道/帧源/可视化器/后处理器/日志门面
...
└── Detection/yoloConfig.json + Detection/model/*.onnx   YOLO 配置与模型
```

依赖方向：`UI → App → Detection/Cameras → Infrastructure/Configuration`。**UI 层禁止接触 OpenCV Mat**；**Detection 层禁止依赖 AppConfig/UI**（所需配置由调用方经参数注入）。
...
2. **事件传出的一律是不可变快照**（列表副本），禁止把内部集合引用泄漏给订阅者。
...
- **Mat ↔ Bitmap 互转只用 `MatExtensions.MatToBitmap/BitmapToMat`**（LockBits + kernel32 RtlMoveMemory 直接 IntPtr→IntPtr 拷贝，1080P 约 0.5~1ms）；**禁止 JPEG 编解码中转**（10~30ms 且画质损失）、禁止逐像素 `Mat.Get<Vec3b>`（百万次 P/Invoke，30~80ms）。
...
| `Detection/MatExtensions.cs` | Mat↔Bitmap 高性能互转（唯一转换入口） |
...
| `Detection/LogManager.cs` | 检测模块日志门面（分级开关 + UI 回调注入） |
...
- 无单元测试框架；以**构建通过 + 冒烟测试**作为验证手段。涉及检测算法的改动，另用 `YOLOTest\test\` 的 Python 脚本或本地图片做对照验证，确保坐标映射/过滤行为不变。
