...
### 新功能：工位操作间门状态检测（门开/关门识别）

**业务**：检测工位操作间的门是否被打开（防静电/安全合规）。COCO 80 类没有"door"，目标检测路线不可行；固定摄像头下采用**关门基准比对**方案，无需训练模型：

- **Detection 类库新增**（随库迁移）：
  - `DoorMonitorAnalyzer`：门区域当前帧与"关门基准图"做**亮度归一化差异比对**（两图各自减去自身均值再比——昼夜/开关灯的整体明暗漂移被消除，只认结构变化）；差异超阈值为候选"门开"，持续 `StateHoldMs`(1.5s) 防抖后翻转并触发 `StateChanged` 事件。误报双保险：门 ROI 被人体框相交时跳过判定（人走过挡门 ≠ 门开）+ 防抖。基准图自动落盘 PNG 重启不丢。
  - `DoorOverlayRenderer`：预览叠加门区域框（绿=DOOR CLOSED / 红=DOOR OPEN / 黄=无基准）。
  - `DoorMonitorOptions`：参数（ROI/差异阈值/防抖/基准路径），注入模式与 ESD 一致。
- **宿主集成**：`doorConfig.json` 配置（复制策略同其他运行配置，构建不覆盖）；`DoorConfig` 配置模型；管道门监测旁路（与 ESD 同一模式，故障只降级）；控制器装配/事件转发/`TryUpdateDoorRoi` 热更新/`SetDoorBaselineFromLatestFrame`（用最近一帧采集基准）。
- **UI**：`标定目标` 按钮（切换拖拽框选归属：静电杆 ↔ 门区域）+ `重设门基准` 按钮（门关着时点击）；门状态日志 `⚠操作间门被打开` / `✓操作间门已关闭`。
- **实现踩坑沉淀**：差异比对最初用"Absdiff 均值减均值差"近似补偿——结构变化本身会抬高均值，把有效差异也扣掉（门开场景 25.6 被扣成 6.4 漏检）；改为逐像素均值归一化（ConvertTo 16S 防下溢）后稳定 23+。防抖哨兵不能用 0（与合法时间戳冲突，从 t=0 开始的会话防抖永不满足），改用 -1。
- **测试**：新增 DoorMonitorTests 分区 6 用例（基准持久化/亮度漂移不误报/结构变化翻转+事件/人遮挡跳过/短暂变化不翻转/配置夹紧与 ROI JSON），127 全绿。
...
### 拍击识别：Hold 时长按业务语义重校（同日现场反馈）

**业务澄清**：实际检测的是工人是否**拍了或摸了**静电杆——拍击的接触时长通常只有 150~400ms，此前 `HoldDurationMs=1000` 会**完全漏掉拍击动作**。

修复：`HoldDurationMs` 默认 **1000→400ms**（`EsdConfig` 与 esdConfig.json 同步，含调参指南注释）。400 的依据：检测循环约 8fps（帧间隔 125ms），400ms ≈ 连续 3 帧命中——单帧抖动防得住，拍击时长覆盖得住；再低（<250ms）接近单帧判定，抖动误报会明显增多。**误报权衡**：手挥过 ROI（200~500ms）也可能触发，但 ROI 仅杆附近小块区域、工位固定镜头下扫过概率低；误报多调回 600~1000，漏拍降 250。同时 UI 结束事件日志补上"本次持续 X 秒"（分析器早已记录清零前最终时长，此前没显示）——现场靠时长区分"拍一下"与"长摸"、审计误判。真机复核：连续快速拍杯 10 次全部识别。
